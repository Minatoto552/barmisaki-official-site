  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/[.07] bg-[#080710]/72 backdrop-blur-xl">
      <div className="mx-auto flex h-[76px] max-w-[1440px] items-center justify-between px-5 sm:px-8 xl:px-[72px]">
        {pathname === '/' ? <span aria-hidden="true" className="block h-11 w-[72px] shrink-0 sm:h-12" /> : <Link href="/" aria-label="BarMisaki ホーム" className="shrink-0"><img src="/barmisaki-logo.png" alt="BarMisaki" width="72" height="48" className="h-11 w-auto object-contain sm:h-12" /></Link>}
        ...
        <div className="hidden items-center gap-3 sm:flex">
          ...
          <Link href="/how-to-join" className="ui rounded-lg bg-[#7b39fc] px-4 py-2.5 text-xs font-semibold shadow-[0_8px_25px_rgba(123,57,252,.32)] transition hover:bg-[#8b51ff]">参加方法</Link>
        </div>
      ...
      {open && <div className="fixed inset-0 z-[90] flex min-h-dvh flex-col bg-[#06050b] p-5 xl:hidden">
        <div className="flex items-center justify-between"><img src="/barmisaki-logo.png" alt="BarMisaki" width="72" height="48" className="h-12 w-auto" /><button onClick={() => setOpen(false)} className="grid size-11 place-items-center rounded-xl border border-white/15" aria-label="メニューを閉じる"><X /></button></div>
        <nav className="my-auto flex flex-col items-center gap-5">{navigation.map((item) => { const active = isActive(pathname, item.href); return <Link key={item.href} href={item.href} onClick={() => setOpen(false)} className={`display text-3xl ${active ? 'text-[#c9a9ff]' : 'text-white'}`}>{item.label}</Link>; })}<a href={externalLinks.officialX} target="_blank" rel="noreferrer" className="mt-3 rounded-xl bg-[#7b39fc] px-8 py-4 font-semibold">公式Xを見る</a></nav>
      </div>}
    </header>
  );
}
