...
export function CastClient({ casts: initialCasts }: { casts: Cast[] }) {
  ...
  const visible = useMemo(() => casts.filter((cast) => category === 'すべて' || (category === 'スタッフ' ? cast.role === 'STAFF' || cast.generation === 'スタッフ' : cast.role !== 'STAFF' && `${cast.generation}${cast.group ? ` ${cast.group}` : ''}` === category)), [casts, category]);
