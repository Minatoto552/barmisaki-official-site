...
export function HomeClient({ casts: initialCasts, news }: { casts: Cast[]; news: NewsItem[] }) {
  ...
  return <main className="home-luxury overflow-hidden bg-[#07060d] text-white">
    ...
    <section className="luxury-section luxury-pickup"><div className="luxury-section-index"><span>03</span> CAST</div><div className="mx-auto max-w-[1240px]">
      ...
      <div className="luxury-full-slider mt-14"><div ref={pickupRef} className="no-scrollbar luxury-slider-track flex snap-x snap-mandatory gap-5 overflow-x-auto pb-4">{pickups.map((cast, index) => <button key={cast.id} onClick={() => setSelectedCast(cast)} className="luxury-cast-slide group"><ImageOrPlaceholder src={cast.images?.length ? cast.images : cast.image} alt={cast.name} focusFace className="transition duration-1000 group-hover:scale-[1.025]" /><span className="luxury-gallery-gradient" /><span className="luxury-cast-count">{String(index + 1).padStart(2, '0')} / {String(pickups.length).padStart(2, '0')}</span><span className="luxury-cast-pick"><Star size={11} fill="currentColor" /> PICK UP</span><span className="luxury-cast-copy"><small>{cast.generation} {cast.group} / {cast.role}</small><strong className="display">{cast.name}</strong><span>VIEW PROFILE <ArrowUpRight size={17} /></span></span></button>)}</div></div>{!pickups.length && <div className="mt-14 border-y border-white/10 py-16 text-center text-sm tracking-[.18em] text-white/35">NEXT SELECTION COMING SOON</div>}
