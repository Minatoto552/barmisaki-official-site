...
export function HomeClient({ casts: initialCasts, news }: { casts: Cast[]; news: NewsItem[] }) {
  ...
  return <main className="home-luxury overflow-hidden bg-[#07060d] text-white">
    ...
    <section id="about" className="luxury-manifesto">
      ...
      <div className="relative z-10 mx-auto grid max-w-[1240px] gap-12 lg:grid-cols-[.8fr_1.2fr] lg:items-end"><div><p className="luxury-kicker">ABOUT BAR MISAKI</p><h2 className="display mt-5 text-[clamp(3rem,6vw,6.5rem)] leading-[.95]">海咲ちゃんが紡ぐ<br /><em className="text-[#c9a1ff]">やさしい夜</em></h2></div><div className="lg:pb-2"><p className="max-w-xl text-base leading-9 text-white/62 sm:text-lg">落ち着いた距離感で会話を楽しめる、海咲ちゃんだけの特別なBar。静かな高揚感と、ここでしか生まれない出会いを。</p><Link href="/how-to-join" className="luxury-text-link mt-8">HOW TO JOIN <ArrowUpRight size={18} /></Link></div></div>
